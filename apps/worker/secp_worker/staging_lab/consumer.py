...
def _refuse(session: Session, item: StagingLabWorkItem, code: StagingWorkFailureCode) -> None:
    """Terminally refuse a claimed work item with a closed, secret-free failure code."""
    _cas_work(
        session,
        item,
        expected_status=StagingWorkStatus.claimed,
        new_status=StagingWorkStatus.refused,
        extra={"failure_code": code, "failed_at": _utcnow()},
    )
    audit.record(
        session,
...
def _claim_candidate(session: Session) -> StagingLabWorkItem | None:
    """Claim one queued work item exclusively, returning it in the ``claimed`` state, or None.

    PostgreSQL: uses ``SELECT ... FOR UPDATE SKIP LOCKED`` so competing workers never contend for
    the same row, then marks it claimed inside the same locked transaction. SQLite / other
    dialects (unit tests): falls back to a portable compare-and-swap conditional UPDATE — a losing
    worker's UPDATE affects zero rows and it backs off. Both guarantee exactly one claimer.
    """
    ...
    if dialect == "postgresql":
        ...
        if candidate is None:
            return None
        candidate.status = StagingWorkStatus.claimed
        candidate.revision = candidate.revision + 1
        candidate.claimed_at = _utcnow()
        session.flush()
        return candidate

    # Portable CAS fallback (SQLite/unit tests): select then conditionally claim.
    candidate = (
        session.execute(
            select(StagingLabWorkItem)
            .where(StagingLabWorkItem.status == StagingWorkStatus.queued)
...
def claim_and_process_one(session: Session) -> uuid.UUID | None:
    ...
    Authoritative-record based: after an exclusive claim (row lock on PostgreSQL, compare-and-swap
    on SQLite), every fact is reloaded from the durable lab and work item and re-validated before
    any fake execution.
    """
    ...
    if not _cas_lab(session, lab, expected_status=queued_status, new_status=running_status):
        _refuse(session, candidate, StagingWorkFailureCode.lifecycle_raced)
        return candidate.id
    ...
    except StagingLabOwnershipError:
        # Blast-radius refusal: revert the lab out of the running phase (closed decision code) and
        # fail the work with a closed failure code. The specific reason is not echoed.
        _cas_lab(
            session,
            lab,
            expected_status=running_status,
            new_status=StagingLabStatus.failed,
            extra={"decision_code": StagingLabDecisionCode.refused_ownership},
        )
        _cas_work(
            session,
            candidate,
            ...
            extra={"failure_code": StagingWorkFailureCode.blast_radius, "failed_at": _utcnow()},
        )
        audit.record(
            session,
    ...
    if not _cas_lab(
        ...
        _cas_work(
            ...
            new_status=StagingWorkStatus.refused,
            extra={
                "failure_code": StagingWorkFailureCode.stale_completion,
                "failed_at": _utcnow(),
            },
        )
        return candidate.id
