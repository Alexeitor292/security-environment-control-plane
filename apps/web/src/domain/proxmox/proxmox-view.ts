...
import { EXECUTION_STATUS_NOT_APPLIED, ISOLATION_CHECK_KEYS } from "../../api/recorded-documents";
// Findings ARE typed on the wire, so their shapes come from the generated contract and not from a
// transcription beside it: `/proxmox/plan` returns `isolation: IsolationFindingOut[]` and
// `/proxmox/readiness` returns `findings: ReadinessFindingOut[]`.
import type {
  IsolationFindingOut,
  ProxmoxGuestOut,
  ReadinessFindingOut,
} from "../../api/generated/openapi";
// `checkStatus` is defined once, in the narrowing layer that produces the findings it reads. A
// second copy here is how the `(observed, ok)` rule ends up enforced in one of two places.
import { type CheckStatus, checkStatus } from "../../api/recorded";
...
/**
 * Tones for the per-check status derived from the `(observed, ok)` pair.
 *
 * Three tones for three outcomes. `not_observed` shares neither the success tone nor the failure
 * tone, because a check that could not be run is not a finding in either direction — and on the
 * wire the pair lives inside an opaque `{ [key: string]: unknown }`, so this is the only place the
 * distinction is kept at all. See `src/api/recorded.ts`.
 */
...
/**
 * The three authorization vocabularies below are OPERATOR PROJECTIONS, not backend enums.
 *
 * The contract publishes `PlanState` (`blocked` | `compiled` | `approved` | `superseded`) and
 * `AuthorizationState` (`blocked` | `plan_not_approved` | `not_authorized` | `authorized`). These
 * split the same facts along the line an operator reads them: what has been decided versus what a
 * server-side gate has since permitted. They are declared here rather than in the transport layer
 * precisely so nobody mistakes them for something the API returns.
 *
 * Apply and destroy keep DISJOINT member names (`apply_authorized` / `destroy_authorized`, never a
 * shared `authorized`) so no screen can render one state under the other's label.
 */
...
/**
 * Isolation holds only when properties were CLAIMED and all of them hold.
 *
 * An empty list returns false. `ProxmoxPlanOut.isolation` is `null` when nothing compiled and an
 * empty array would say "no isolation properties were claimed" — neither is a proof of isolation,
 * and `[].every(...)` is `true`, so the length check is the whole guard.
 */
export function isolationHolds(findings: readonly IsolationFindingOut[]): boolean {
  return findings.length > 0 && findings.every((f) => f.holds);
}

/** Same rule for readiness: nothing assessed is not the same as everything satisfied. */
export function readinessSatisfied(findings: readonly ReadinessFindingOut[]): boolean {
  return findings.length > 0 && findings.every((f) => f.met);
}

/**
 * Guest rows from the TYPED endpoint projection rather than from the opaque topology document.
 *
 * `ProxmoxGuestOut` is what `/proxmox/topology` publishes per guest: identity, placement, MACs,
 * addresses and the ownership generations. It deliberately does NOT carry sizing (`cpu_cores`,
 * `memory_mb`, `disks`) or per-NIC detail, which exist only inside `ProxmoxTopologyOut.topology`.
 * A screen that needs those reads the compiled document via `guestRows`; a screen that needs
 * identity and placement should use this and stay inside the typed contract.
 *
 * The three addresses stay three. `observedAddress` is `null` when the provider was never
 * observed AND when it was observed and reported none — `observed` is what tells them apart.
 */
export interface PublishedGuestRow {
  guestRef: string;
  name: string;
  kind: string;
  vmid: number | null;
...
export function publishedGuestRows(guests: readonly ProxmoxGuestOut[]): PublishedGuestRow[] {
  return guests.map((g) => ({
    guestRef: g.guest_ref,
    name: g.name,
    kind: g.kind,
    vmid: g.vmid ?? null,
    node: g.node_name ?? null,
    template: g.template_ref ?? null,
    macs: [...(g.mac_addresses ?? [])],
    publishedAddress: g.address.published_address,
    probeAddress: g.address.probe_address ?? null,
    probeIsDistinct: g.address.probe_is_distinct,
    observedAddress: g.address.observed_address ?? null,
    observed: g.address.observed,
    teamRef: g.team_ref ?? null,
    generation: g.generation ?? null,
    operationGeneration: g.operation_generation ?? null,
  }));
}
