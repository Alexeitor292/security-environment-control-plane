...
#:
#: The four installation witnesses mark that a stage's body EXECUTED, not that it succeeded. That
...
#: at all — and it is only the second case a witness can see. Tying a witness to success would
#: collapse the two and make a silently-absent stage indistinguishable from a failing one. Their
#: names describe the EXECUTION for the same reason the rename above happened: the property already
#: has a check id, and a witness that reads like the property invites exactly that misreading.
CONTAINER_TIER_STAGES: tuple[str, ...] = (
    ...
    "worker_image_probed",
    "release_built",
    "packages_stage_driven",
    "controller_stage_driven",
    "worker_install_stage_driven",
)
...
EXPECTED_CONTAINER_NODES_BY_MODULE: dict[str, int] = {
    ...
    # release shape 2, packages 4, controller 5, worker_install 7, document-level 2.
    "test_acceptance_container_installation.py": 20,
}
