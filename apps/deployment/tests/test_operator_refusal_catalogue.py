...
and covers **27 of the 88** catalogued codes: 26 in ``verify.py``, and exactly one —
``operator_consumer_active`` — in ``queue_check.py``.

Of the other 61, measured by asking whether the literal appears in each module outside the
...
* **33 are raised elsewhere** — ``profile.py``, ``manifest.py``, ``compositions.py``,
  ``runtime_seams.py``, ``cli.py``, ``production_context.py``, ``runner.py`` and ``secp_worker``.
  This bucket includes the four POSIX/backend refusals surfaced by the CLI's bounded top-level
  guard, which is exactly the sort of addition a scan over two files cannot see.

This 23/5/33 split is re-measured on every run by
