...
is exactly ``7c2f4b8d1a6e``. It is one of the targets of the exact-head PostgreSQL no-skip CI
...
def test_live_alembic_head_is_exactly_7c2f4b8d1a6e(pg_config) -> None:
    cfg, engine = pg_config
    command.upgrade(cfg, "head")
    assert _live_head(engine) == HEAD
