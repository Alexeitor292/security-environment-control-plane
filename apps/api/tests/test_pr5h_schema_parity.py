...
HEAD = "7c2f4b8d1a6e"  # SECP-M1-B sole head (provider-execution credential reference)
SELECTED_WORKER = "e3b7a9c25f41"  # HEAD's parent (the operation's selected worker, ADR-030 c2)
PR5H_B2 = "a1d4f7c2e9b6"  # SELECTED_WORKER's parent (PR5H-B2 controller-identity activation)
...
def test_7c2f4b8d1a6e_is_the_sole_head_with_the_linear_chain() -> None:
    ...
    #         -> 7c2f4b8d1a6e
    assert script.get_revision(HEAD).down_revision == SELECTED_WORKER
    assert script.get_revision(SELECTED_WORKER).down_revision == PR5H_B2
...
def test_downgrade_removes_only_the_four_pr5h_tables(tmp_path, monkeypatch) -> None:
    ...
    command.downgrade(cfg, PR5F_BASE)
