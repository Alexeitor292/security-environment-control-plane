...
schema-wrong marker seals. Off production the env flag still enables (test/dev compatibility). The
POSIX filesystem-posture check is skipped off POSIX (these tests run on any host).
"""
...
def test_valid_marker_loads(tmp_path):
...
def test_bad_marker_bytes_seal(tmp_path, raw):
    assert load_valid_marker(path=_write(tmp_path, raw)) is None


def test_non_canonical_marker_seals(tmp_path):
    import json
...
def test_env_cannot_enable_the_signer_in_production(tmp_path):
...
def test_valid_marker_enables_in_production(tmp_path):
    client = build_enrollment_offer_signer(
...
def test_malformed_marker_seals_in_production(tmp_path):
    client = build_enrollment_offer_signer(
...
def test_marker_binding_mismatch_is_detectable(tmp_path):
    marker = load_valid_marker(path=_write(tmp_path, _marker_obj()))
