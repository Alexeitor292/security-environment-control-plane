...
``FakeProcessExecutor`` (returning safe fixture ``show -json``) and a
``FakeSecretResolver`` — no real binary, provider, network, or endpoint. Covers: full
approved dry-run → approve → apply → destroy; apply refused without / on drift from an
approved change set; destroy requires its own approved change set; Temporal-only (inline
refused) and mode/real-setting gates; drift invalidation; simulator unchanged; and no
secret leakage.
"""
...
def _run(session, manifest, kind, *, actions=("create",), resolver=None, dispatch="temporal"):
    return run_real_provisioning(
        session,
        manifest.id,
        kind,
        executor=_exec(manifest, actions=actions),
        settings=REAL_ON,
...
def test_full_real_lab_lifecycle_with_explicit_approval(session, principal, lab_env):
    ...
    dry = _run(session, m, ProvisioningOperationKind.dry_run)
    session.commit()
    ...
    applied = _run(session, m, ProvisioningOperationKind.apply, resolver=_resolver())
    session.commit()
    ...
    ddry = _run(session, m, ProvisioningOperationKind.destroy_dry_run, actions=("delete",))
    session.commit()
    ...
    destroyed = _run(
        session, m, ProvisioningOperationKind.destroy, actions=("delete",), resolver=_resolver()
    )
    session.commit()
...
def test_apply_refused_without_approval(session, principal, lab_env):
    env = lab_env()
    _run(session, env.manifest, ProvisioningOperationKind.dry_run)  # pending, not approved
    session.commit()
    with pytest.raises(ProvisioningRefusedError, match="none is approved"):
        _run(session, env.manifest, ProvisioningOperationKind.apply, resolver=_resolver())
...
def test_apply_refused_on_regenerated_dry_run_mismatch(session, principal, lab_env):
    env = lab_env()
    m = env.manifest
    _run(session, m, ProvisioningOperationKind.dry_run, actions=("create",))
    session.commit()
    approval = _pending(session, m.id, ProvisioningOperationKind.apply)
    approvals.approve_change_set(session, principal, approval.id, "approved original")
    session.commit()

    # The regenerated dry run now yields DIFFERENT plan actions → different hash.
    with pytest.raises(ProvisioningRefusedError, match="differs from the approved"):
        _run(session, m, ProvisioningOperationKind.apply, actions=("update",), resolver=_resolver())
...
def test_destroy_refused_without_destroy_approval(session, principal, lab_env):
    env = lab_env()
    m = env.manifest
    _run(session, m, ProvisioningOperationKind.dry_run)
    session.commit()
    approvals.approve_change_set(
        session, principal, _pending(session, m.id, ProvisioningOperationKind.apply).id, "ok"
    )
    session.commit()
    _run(session, m, ProvisioningOperationKind.apply, resolver=_resolver())
    session.commit()
    # An apply approval is not enough; destroy needs its own approved change set.
    with pytest.raises(ProvisioningRefusedError, match="none is approved"):
        _run(
            session, m, ProvisioningOperationKind.destroy, actions=("delete",), resolver=_resolver()
        )
...
def test_toolchain_profile_disabled_after_approval_refuses(session, principal, lab_env):
    ...
    session.commit()
    with pytest.raises(ProvisioningRefusedError, match="not active|drift|mismatch"):
        _run(session, m, ProvisioningOperationKind.apply, resolver=_resolver())
...
def test_scope_policy_drift_after_manifest_refuses(session, principal, lab_env):
    ...
    with pytest.raises(ProvisioningRefusedError, match="scope_policy has drifted"):
        _run(session, env.manifest, ProvisioningOperationKind.dry_run)
...
def test_non_isolated_lab_profile_cannot_reach_real_path(session, principal, lab_env):
    ...
    with pytest.raises(ProvisioningRefusedError, match="toolchain profile"):
        _run(session, manifest, ProvisioningOperationKind.dry_run)
...
def test_no_secret_leaks_in_records_or_audit(session, principal, lab_env):
    ...
    assert "fake-lab-token" not in audit_blob and "super-secret" not in audit_blob
