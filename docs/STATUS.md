...
| **SECP-006 — Enterprise Readiness** | `partially-implemented`, `production-blocked` | Foundations exist: organization/tenant scoping, RBAC foundations, immutable audit, and production safety guards (the `Settings` production validator hard-refuses dev auth, inline dispatch, fake provisioning, and the OpenTofu subprocess). Backend OIDC bearer verification is now implemented (ADR-017, see §D); interactive browser login (PKCE — OIDC-B), additional IdPs, advanced RBAC, HA, backup/restore, plugin signing, cloud-provider plugins, and full enterprise operations do **not** exist yet. |
...
| OIDC bearer authentication (backend) | `implemented` | Cryptographic bearer-token verification (RS256, issuer/JWKS/audience/time/subject) mapping the exact `sub` to a pre-provisioned user; org/permissions are DB-owned; token roles/groups/email grant nothing (ADR-017). Interactive browser login (PKCE) is NOT implemented — the OIDC-B slice. The dev fallback remains, non-production only. |
...
- **OIDC bearer-token verification is implemented (backend only).** A presented bearer token is cryptographically verified (RS256 signature against the configured issuer's JWKS, exact issuer, required audience, time claims, bounded subject) and its exact `sub` maps to one pre-provisioned user; organization and permissions are database-owned and token roles/groups/email grant nothing. **Interactive browser login (Authorization Code + PKCE) is still not implemented** — that is the OIDC-B slice.
...
- ~~**Real OIDC bearer-token verification**~~ — **done (backend, ADR-017).** Bearer tokens are cryptographically verified and mapped to pre-provisioned users; what remains is the **interactive browser login** (Authorization Code + PKCE, session/token lifecycle, logout) — the OIDC-B slice — plus additional IdPs and advanced RBAC.
...
2. Production OIDC authentication *(backend bearer verification complete — ADR-017; interactive browser login / PKCE is the remaining OIDC-B slice)*
