...
**Browser Authorization Code + PKCE login (ADR-018 / OIDC-B) — implemented:** the placeholder browser login is replaced by a real, provider-neutral OpenID Connect **Authorization Code flow with PKCE (S256)** through the public `secp-web` client (no client secret), using the maintained `oidc-client-ts` library. The browser reads a **public, secret-free** `GET /api/v1/auth/config` (server-derived `mode`, issuer, public client id, audience, fixed `openid profile email` scope with **no** `offline_access`, and fixed relative callback paths), runs the code+PKCE flow, and sends the resulting **access token** as `Authorization: Bearer` to the API — the ID token is never an API credential and never grants org/role/permission. `/api/v1/me` is the **authoritative** browser identity; token claims are never used for SECP authorization or organization selection. Tokens are **session-scoped only** (no localStorage/IndexedDB/cookies/DB persistence, no service-worker cache); access-token expiry clears local state and requires a fresh interactive login (no silent renewal, no `offline_access`, no refresh-token session). Every application route is guarded (no protected content or domain API call before authentication resolves); an invalid callback, `state`/`nonce` mismatch, token error, or API **401** fails closed to `/login` with a sanitized same-origin return path, a **403** keeps the session, and a **503** shows a provider-unavailable message. Logout clears local state and invokes the provider end-session when available. Frontend auth errors are bounded categories that never expose a token, code, state, nonce, verifier, claim, subject, or provider detail. The development fallback remains available **only** when the backend reports `mode = dev_fallback` (non-production). This slice adds **no** backend session/BFF, token database, JIT provisioning, token-derived RBAC, or multi-issuer support, and unseals **no** provisioning or infrastructure; **OIDC-C** production deployment and operational runbooks remain future work.
...
| **SECP-006 — Enterprise Readiness** | `partially-implemented`, `production-blocked` | Foundations exist: organization/tenant scoping, RBAC foundations, immutable audit, and production safety guards (the `Settings` production validator hard-refuses dev auth, inline dispatch, fake provisioning, and the OpenTofu subprocess). Backend OIDC bearer verification (ADR-017) and browser Authorization Code + PKCE login (ADR-018) are now implemented (see §D); OIDC-C production deployment/runbooks, additional IdPs, advanced RBAC, HA, backup/restore, plugin signing, cloud-provider plugins, and full enterprise operations do **not** exist yet. |
...
- ~~**Real OIDC bearer-token verification + interactive browser login**~~ — **done (ADR-017 backend bearer + ADR-018 browser Authorization Code + PKCE).** Tokens are cryptographically verified and mapped to pre-provisioned users, and the browser obtains them via the public-client PKCE flow. What remains is **OIDC-C** production deployment/runbooks, additional IdPs, and advanced RBAC.
...
2. Production OIDC authentication *(backend bearer verification — ADR-017; browser Authorization Code + PKCE login — ADR-018; both complete. OIDC-C production deployment/runbooks remain)*
